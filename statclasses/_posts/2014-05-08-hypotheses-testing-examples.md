...
```r
data <- rbind(c(4, 4, 4), c(6, 4, 7))  #введемо таблицю спряженості по рядкам за допомогою функції rbind(), аналізуємо стовпчики 1, 3 і 4.
...
```r
data[, 1] <- c(8, 0)  #замінюємо стовпчик результатів інтактної групи на стовпчик діазепаму
fisher.test(data)
```
...
##ПРИКЛАД 2. 
Повернемось до *Student’s Sleep Data* [\[2\]](2). Встановимо тепер, чи статистично значимо відрізняють результати випробувань двох снодійних препаратів.
...
Отриманий графік не закриваємо, продовжуємо ввід команд:


```r
set.seed(29)  #ініціалізуємо генератор випадкових чисел заданим ключем – це робиться для того, щоб отримати однакові результати при усіх виконаннях #наступного коду
```
...
```r
for (i in 1:3) boxplot(c(a, b)[sample(length(c(a, b)))] ~ c(rep(1, length(a)), 
    rep(2, length(b))))
```
...
Щоб порівняти стабільності продажів, на графіках ми порівнюємо різниці в міжквартильній відстані. Ця різниця для першого графіку (на ньому продажі препаратів А і Б) майже не відрізняється (і навіть менша) від різниці на другому та третьому графіках. Тому робимо висновок, що досліджувані лікарські препарати не відрізняються стабільністю продажів. Крім того, помітно що різниця між медіанами на першому графіку достатньо сильно відрізняється від різниці на інших графіках. Виконавши тест Уілкоксона (чому саме Уілкоксона?),
...
[1]: Цубанова Н.А., Штриголь С.Ю. Дослідження антидепресивної та анксіолітичної дії спіроциклічного похідного оксіндолу. Вісник фармації. – 2011. - №1. – С. 77-80.

[2]: Student The probable error of the mean // Biometrika. -1908. - 6. - P. 20.
